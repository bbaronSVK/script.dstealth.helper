...
__transPath = xbmc.translatePath if H.Helper.getKodiVersion() < 19 else xbmcvfs.translatePath
...
def get(cache_key:str, cachelife_seconds:int, freshcall_func, *args, **argz):
    """
    Attempts to retrieve data from cache if it exists and isn't expired
    else attempts to retrieve new data.

    Returns an Object or None if errored.
    ...
    try:
        if success and (response != '') and (response != []) and (response != {}) and (response != [{}]):
            ...
            __logd("Request failed or response was empty.")
    except Exception as e:
        if H.DEBUG:
